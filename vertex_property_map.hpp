...
class vertex_property_map
{
	private:
		...
		typedef std::map<adj_matrix::vertex_t, T> PMap;
	private:
		...
		class Collector
		{
			...
			public:
				Collector(adj_matrix::vertex_list & vec,
					const Predicate & pred)
					: vec(vec), pred(pred)
				{}
	...
	public:
		...
		T & operator[](adj_matrix::vertex_t v)
		{
			return p[v];
		}
		...
		/// vertex. This operator does not check boundaries.
		const T & operator[](adj_matrix::vertex_t v) const
		{
			return p[v];
		}
		...
		template <class Predicate>
		void collect_if(adj_matrix::vertex_list & vec,
			const Predicate & pred)
		{
