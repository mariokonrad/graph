...
public:
	...
	void test_dfs(const graph::vertex_list & expected, graph::vertex start) const
	{
		graph::vertex_list v;
...
TEST_F(Test_dfs, graph_start_0) { test_dfs(graph::vertex_list{0, 1, 3, 2, 4}, 0); }

TEST_F(Test_dfs, graph_start_1) { test_dfs(graph::vertex_list{1, 3, 0, 2, 4}, 1); }

TEST_F(Test_dfs, graph_start_2) { test_dfs(graph::vertex_list{2, 3, 0, 1, 4}, 2); }

TEST_F(Test_dfs, graph_start_3) { test_dfs(graph::vertex_list{3, 0, 1, 2, 4}, 3); }

TEST_F(Test_dfs, graph_start_4) { test_dfs(graph::vertex_list{4}, 4); }
}
